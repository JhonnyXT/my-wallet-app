...
- **Falso positivo genérico (todos los bancos): recordatorios/invitaciones a pagar una factura se detectaban como gasto real.** Ej. Nu envía "Tienes un pago por $X. Completa tu pago de forma fácil y segura en tu app Nu." para un pago de servicios (UNE-EPM) que el usuario NO ha confirmado todavía — el parser lo tomaba como un débito ya ocurrido solo porque contenía un monto y la palabra "pago". Corregido en `src/utils/notificationParser.ts`: se quitó "pago" (sustantivo ambiguo) de `expenseKeywords` y se agregaron patrones de ruido genéricos (`tienes un pago pendiente/por`, `completa tu pago`, `finaliza tu compra`, `recuerda pagar`, `factura pendiente`, etc.) al arreglo `NOISE_PATTERNS`, que corre ANTES de elegir el patrón específico del banco — por eso el fix aplica a los 15 bancos de la whitelist, no solo a Nu. Las transferencias recibidas de otro banco siguen detectándose como ingreso normalmente vía `incomeKeywords` (recibiste, te enviaron, transferencia recibida, etc.) — eso no cambió.
