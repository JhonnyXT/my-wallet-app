...
- **Qué se tomó de `ui-ux-pro-max` y qué se descartó** (para no repetir el mismo proceso de prueba y error si se vuelve a tocar el diseño):
  ...
  - Se **descartó la paleta de color genérica** que devolvió el generador (`#1E40AF` azul + `#059669` verde sobre fondo `#0F172A`, estilo "Dark Mode OLED"). En su lugar se usaron los colores reales de MyWallet (`#135BEC` accent, `#22C55E` verde, `#EF4444` rojo — los mismos tokens de `src/theme/index.ts`), para que la landing se sienta como la app real y no como una plantilla genérica de fintech.
