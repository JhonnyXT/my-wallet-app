...
21. [Bitácora de gotchas y deuda resuelta](#21-bitácora-de-gotchas-y-deuda-resuelta)
...
- **`descriptionExtractor.ts`** — `extractDescription()` limpia el texto de la notificación (monto, saldo, fechas, frases fijas del banco) y devuelve la oración completa restante (no un fragmento tipo keyword) — sigue viva en `ParsedTransaction.description`, usada solo para adivinar la categoría (`guessCategoryEmoji`, más palabras clave disponibles) sobre el texto crudo. `shortenDescription(description, isExpense, amount)` (firma con `amount` desde 2026-09-23) la reduce a una etiqueta corta que **siempre incluye el monto** ("Compra en RAPPI CO · $ 45.000", "Recibiste de Juan Pérez · $ 80.000") — usada en los 3 lugares que muestran/guardan la descripción: el cuerpo de la notificación push (`notifyBankTransaction`), la tarjeta y la nota guardada de `notification-review.tsx` (`pendingToReview()`), y la nota prellenada del flujo `notification-detect` (`app/_layout.tsx` → `prefillExpenseFromPendingItem()`). Ver sección "Confidence-aware push" más abajo.
...
- **Bug de dictado perdido en silencio, corregido 2026-09-23:** si el timer propio de 2s de silencio llamaba `SpeechModule.stop()` y el módulo nativo (`expo-speech-recognition`) terminaba con `"end"` sin emitir antes un `"result"` con `isFinal:true` (posible en algunos dispositivos/versiones de Android, no garantizado por la librería), el listener de `"end"` solo hacía `setStatus("idle")` — nunca llamaba a `handleDone()`, el dictado se perdía sin navegar a ningún lado. Fix con dos refs: `transcriptTextRef` (transcript acumulado, actualizado en cada `"result"`) y `silenceStopRef` (true solo si el próximo `stop()` viene del timer de silencio, no de un stop manual vía el botón de pausa). El listener de `"end"` ahora recupera el dictado pendiente (`handleDone(pending)`) cuando `silenceStopRef` es true; un stop manual sigue sin auto-enviar. Guard anti doble-procesamiento: `transcriptTextRef.current = ""` se limpia ANTES de `handleDone` cuando `isFinal` dispara el flujo normal desde `"result"`, para que un `"end"` que llegue justo después no reprocese el mismo texto.
...
> (hook roto) fueron **eliminados del repo** — ver §21 (Bitácora).
...
## 21. Bitácora de gotchas y deuda resuelta

Historial completo (con fechas, intentos revertidos y el porqué de cada cambio) que antes vivía en
`AGENTS.md`. `AGENTS.md` se carga en cada sesión de agente, así que ahí solo quedan las reglas
vigentes en presente; el detalle histórico se consulta aquí o en `git log`. Esta sección es un
registro, no se mantiene al día: si algo aquí contradice a `AGENTS.md` o al código, manda lo actual.

### Gotchas (texto íntegro al 2026-09-24)
...
- **`src/utils/notificationParser.ts` ahora es una carpeta** (`src/utils/notificationParser/`), un módulo por responsabilidad: `types.ts`, `intentClassifier.ts` (clasifica `otp` / `security_alert` / `payment_reminder` / `marketing` / `app_update` / `possible_transaction` ANTES de intentar parsear nada — reemplaza el antiguo array plano `NOISE_PATTERNS`), `amountExtractor.ts`, `directionClassifier.ts`, `descriptionExtractor.ts`, `bankPatterns.ts` (un patrón por banco), `parseNotification.ts` (orquesta el pipeline, API pública) y `fixtures.ts` (casos reales/sintéticos con resultado esperado). El import externo `@/src/utils/notificationParser` sigue funcionando igual (resuelve a `index.ts`), ningún consumidor cambió.
  ...
  - **Título sin monto; cuerpo con etiqueta corta de la acción, que ahora incluye el monto ella misma** (2026-07-23, corregido 2026-09-23). El título sigue siendo una etiqueta corta ("Nuevo gasto detectado — Bancolombia" / "¿Nuevo ingreso? — Nequi"), sin el monto. El cuerpo usa `shortenDescription()` (`descriptionExtractor.ts`), que **desde 2026-09-23 recibe un tercer parámetro `amount: number` y SIEMPRE lo incluye al final del string** (formateado con `formatCOP()`), ej. `"Compra en RAPPI CO · $ 45.000"` / `"Recibiste de Juan Pérez · $ 80.000"` — antes (2026-07-23) `shortenDescription` devolvía solo verbo + contraparte sin monto y `notifyBankTransaction()` concatenaba `amtStr` aparte (`"${amtStr} · ${shortDesc}"`); esa concatenación se quitó porque duplicaba el número. `amtStr` (ahora también vía `formatCOP()`, ya no un regex propio) solo sobrevive como fallback en los 2 casos sin descripción. El verbo (Compra/Pagaste/Enviaste/Retiro vs Recibiste/Ingreso/Consignación) lo decide **`isExpense`** (ya clasificado por `classifyDirection`), no una keyword suelta del texto — bancos distintos frasean la misma acción distinto (ej. "enviaron" es ingreso en Nequi cuando alguien te manda plata, sería gasto si el usuario envía), así que solo `isExpense` decide la dirección; las keywords dentro de cada rama solo eligen el verbo más específico. **Gasto: "Pago" se separó en dos verbos (2026-09-23)** — "Pagaste" (`pag...`, pagar en un comercio) vs "Enviaste" (`envi(aste|ó)`, mandarle plata a otra persona), simétrico con "Recibiste" del lado ingreso. **Ingreso: nuevo caso "sujeto primero" (2026-09-23)** — transferencias entrantes donde el remitente aparece ANTES del verbo ("Bancolombia te envió...", "Juan Pérez te transfirió...") ahora dan "Recibiste de {remitente}"; antes caían al genérico "Ingreso" sin decir quién mandó la plata (bug real, es el fixture "Nu — transferencia entrante de otro banco" de `fixtures.ts`). Usa un lookahead `(?=\s|$)` en vez de `\b` al final del regex, porque en JS `\w` es solo ASCII y una vocal acentuada ("envió", "transfirió") no cuenta como carácter de palabra — `\b` nunca coincide justo después. La contraparte (cuando no aplica el caso "sujeto primero") se extrae de la última frase preposicional ("en X"/"a X"/"de X"/"Comercio: X") del texto ya limpio. `extractDescription()`/`bankPatterns.ts` no cambiaron — la descripción **completa** capturada del banco sigue viva en `ParsedTransaction.description`, pero **ya no es lo que se muestra/guarda como descripción en los otros dos consumidores** (ver gotcha siguiente).
  - **`shortenDescription()` ahora se usa en 3 lugares con el MISMO resultado, no solo en la notificación push (2026-09-23).** Antes, la pantalla de revisión (`notification-review.tsx` → `pendingToReview()`) y la nota prellenada al abrir directo desde una notificación (`app/_layout.tsx` → `prefillExpenseFromPendingItem()`) mostraban `item.description` crudo (`extractDescription()`, casi tal cual la notificación del banco — a veces muy verboso, ej. "Su tarjeta debito realizo una compra Comercio: NETFLIX $38.900"). Ahora los tres (`notifyBankTransaction()`, `pendingToReview()`, `prefillExpenseFromPendingItem()`) usan `shortenDescription()` para lo que se muestra/guarda como descripción — la categoría se sigue adivinando (`guessCategoryEmoji`) sobre el texto **crudo** (más palabras clave disponibles, ej. nombres de comercio), no sobre la versión corta; solo lo que se muestra/guarda como descripción cambió.
- **Bug real encontrado y corregido: la detección de categoría (texto/voz/notificaciones) nunca filtraba por tipo gasto/ingreso, y las categorías predefinidas de la app tienen colisiones reales de keywords entre ambos tipos (2026-09-23).** `"mensualidad"` está en la categoría de gasto "Suscripciones" (📱) Y en la de ingreso "Salario" (💼); `"regalo"` está en la de gasto "Regalos" (🎁) Y en la de ingreso "Extra" (🎁, mismo emoji) — `categoryPresets.ts`. Sin filtrar por tipo, un ingreso real con esas palabras (ej. "Consignación mensualidad Bancolombia") podía etiquetarse con una categoría de GASTO, la primera coincidencia del array sin importar si la transacción era realmente un ingreso.
  - `guessCategoryEmoji(description, userCats?, isExpense?)` (`src/constants/theme.ts`) gana un 3er parámetro opcional `isExpense`. Si se pasa, filtra `userCats` a solo categorías de ese `type` antes de buscar keywords, y usa el mapa legacy correspondiente (`EXPENSE_CATEGORY_MAP`/`INCOME_CATEGORY_MAP`, nuevos — el viejo `CATEGORY_MAP` mezclado queda `@deprecated`, solo como fallback sin `isExpense` conocido). Sin `isExpense` (`undefined`), sigue buscando en ambos tipos como antes (comportamiento legacy preservado). Llamadores actualizados: `app/notification-review.tsx` → `pendingToReview()` pasa `item.isExpense`; `app/_layout.tsx` → `prefillExpenseFromPendingItem()` pasa `item.isExpense`; `src/utils/nlp.ts` → `parseExpenseInput()` pasa `true` fijo (esa función es siempre flujo de gasto). Test nuevo: `src/constants/theme.test.ts` (6 tests).
  - `src/utils/voiceParser.ts` (dictado por voz) tiene el mismo bug, arreglo simétrico: su `CATEGORY_MAP` local (array de `{keywords, emoji, name}[]`, distinto del de `theme.ts`) ahora tiene un campo `type: "expense"|"income"` obligatorio en cada entrada (12 entradas, 7 gasto + 5 ingreso). `extractCategory(text, userCats?, isExpense?)` gana el mismo 3er parámetro opcional. `processVoiceInput()` calcula `isExpense` (vía `extractIsExpense`) ANTES de llamar a `extractCategory` y se lo pasa — antes eran dos llamadas independientes sin comunicarse.
  - **Bug relacionado en la MISMA función, encontrado escribiendo los tests:** `extractIsExpense()` tenía `"mensualidad"` en su lista de `incomeKeywords`, chequeada INCONDICIONALMENTE antes que la de gasto — una frase como "pagué mi mensualidad de Netflix" (gasto explícito, verbo "pagué") se clasificaba como INGRESO solo por contener "mensualidad", sin importar el verbo de gasto presente. Se quitó `"mensualidad"` de `incomeKeywords` (es ambigua); `"salario"`/`"sueldo"`/`"nomina"`/`"recibi"` ya cubren el caso real de ingreso sin esa ambigüedad. Tests ampliados: `src/utils/voiceParser.test.ts` (+4 tests en una `describe` nueva "categoría NO cruza gasto/ingreso").
